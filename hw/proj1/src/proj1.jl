...
prior_β = Frailty.Prior_β(zeros(2), eye(2)*10^3, sym(inv(X'X))*1.)
prior_λ = Frailty.Prior_λ(.001,.001)
prior_α = Frailty.Prior_α(.001,.001,.1)
prior_η = Frailty.Prior_η(.001,.001)

Frailty.fit(t,X,v,group,prior_β,prior_λ,prior_α,prior_η,1000,1000)
